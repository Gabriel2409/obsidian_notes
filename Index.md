...
- When using an index,
  - the value can be a pointer to the location of the row (the rows are stored on the **heap file**). In this case, when updating the value, you just need to update the pointer in the heap file. Note that if the new value is larger in the heap file, it is common to append the new value at the end and leave a forwarding pointer at the previous location
